...
}//returning userSnapshot since the data lives in userSnapshot since docRef is just a pointer to that space
return userSnapshot;
};
...
export const getCurrentUser = () =>{
    return new Promise((resolve,reject) => {
        ...
        const unsubscribe = onAuthStateChanged(
            auth,
            (userAuth) => {
                unsubscribe();//If we dont close it then there will be a memory leak
                resolve(userAuth);
            },
            //The third argument which it take is the error that it takes....
            //Reject gives the thing that is error
            reject
        );
    });
};